...
The core catalog includes Netflix, YouTube, Disney+, and Spotify. Spotify uses its official Web Player, a dedicated persistent session, exact player and account origin boundaries, direct allowlisted search paths, declared phone search text-entry targets, native media keys, and a local TV Mode presentation. Signed-out launches show one large remote-selectable sign-in action before provider content. Signed-in launches expose horizontal Home/Search/Your Library navigation, recommendations, tracks, and the persistent provider player through a visible-target D-pad model. Spotify-owned secure session cookies may be retained locally for up to 30 days, excluding transient security-flow cookies, so users are not asked to authenticate on every launch; explicit service-data clearing and provider logout still remove access. Existing profiles may add Spotify from the App Store without changing their saved lineup; new profiles receive the current core catalog. The experimental catalog includes Prime Video, Hulu, HBO Max, Peacock, Paramount+, Apple TV, Plex, and Twitch. These use official HTTPS entry points, exact-origin navigation boundaries, and separate persistent partitions. They remain disabled by default and do not advertise search, playback observation, or compatibility until per-platform qualification is complete.

When Spotify is open, the ordinary Home action preserves that renderer as the single background audio session. Home exposes remote-sized Previous, Play/Pause, Next, and Return to Spotify controls while Spotify retains playback and queue ownership. Other services continue to close on Home, and Force Home always closes the background session.
