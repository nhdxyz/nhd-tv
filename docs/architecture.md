...
Castlabs Electron for Content Security is the leading candidate because it provides a consistent Chromium runtime and a supported path for installing Widevine. Stock Electron is not assumed to satisfy commercial DRM requirements, including the protected-content requirement documented for Spotify's Web Player. Tauri remains unattractive for this product because it uses different system webviews across Windows, macOS, and Linux.
...
The host holds a single-instance lock. A repeat launch, a macOS dock activation,
or completion of a new-window load restores, shows, activates, and focuses that
window. This keeps a fullscreen window on the remembered television display
from appearing to vanish behind another application or in an inactive Space.
...
Each service adapter owns exact allowed navigation origins and may also declare a narrow provider-owned HTTPS host family when an authentication flow moves between service subdomains. Host-family matches require the provider root or a dot-delimited subdomain, reject custom ports, and never expand media-key-system or fullscreen permissions. Netflix, YouTube, Disney+, and Spotify opt into only their own provider hosts; Google's account origin stays exact instead of trusting all of `google.com`. Lookalike suffixes, unrelated identity providers, non-HTTPS schemes, and undeclared origins remain blocked.
...
The shell's Gamepad API adapter watches connection events and polls only while at least one standard-mapped controller is connected. It applies a left-stick dead zone and bounded directional repeat, then submits only the normalized action vocabulary over a sender-validated IPC channel. The main process applies the same overlay, shell, and active-service routing used by phone commands. Hardware media keys and documented Command-or-Control plus Shift fallbacks normalize through that same route. Playback and seek actions reach active services as native Electron accelerator or keyboard events; Netflix and Spotify play/pause use Space because their web players document or require that path. Spotify's transport pair maps to its documented Up/Down previous/next shortcuts so the generic Left/Right events cannot accidentally add a track to the library or queue. Media actions never add player selectors or directly manipulate player DOM. Volume actions are separated from service routing and adjust the host operating system output in bounded five-percent steps, with an explicit unavailable result if the platform route fails. This avoids making Netflix or another service responsible for television volume, while HDMI/CEC behavior still requires platform qualification. The host safely reports that an app must be open when a shell-only playback action has no target. Raw controller state is never exposed to a service page.
...
YouTube additionally loads the repository's local Manifest V3 TV Mode extension in its persistent service session. The extension owns the YouTube-specific 10-foot presentation, route-aware card navigator, and per-route focus restoration. A cancelable DOM event hands normalized host actions to that navigator; if it is disabled, absent, in player-first mode, or declines the action, the conservative host spatial layer and native-key fallback remain unchanged. A separate cancelable configuration event applies device-wide TV Mode, interface-size, and safe-area preferences and marks the host as sole controller-input owner, so the content script does not run a second Gamepad poller. The extension never receives shell IPC or Node.js access.

Spotify uses the same isolated-extension boundary for a smaller provider-specific TV layer. The host removes only the `Electron/<version>` product token from Spotify's view-level user agent so Spotify serves its supported Chromium Web Player. Spotify's own HTTPS navigation remains in the one active service view, including account pages and playback actions that the desktop site would otherwise request as a new window. The extension supplies the top navigation, signed-out entry screen, ten-foot CSS, stable card/track annotations, and visible-geometry D-pad navigator; it does not call Spotify APIs, access shell IPC, or replace the provider's player.

Spotify's login depends on secure cookies that the provider may issue without an expiration date. Electron documents those as session cookies that are not retained between application sessions even when the partition itself is persistent. Inside only the isolated Spotify partition, NHD-TV converts secure, Spotify-owned, non-transient session cookies to local cookies with a maximum 30-day expiration. CSRF, OAuth state, nonce, flow, CAPTCHA, PKCE, and challenge cookies are excluded; values are never logged or exported; provider deletion still removes the cookie. The session also flushes DOM storage and cookies after navigation and before host lifecycle checkpoints. Clearing Spotify data remains the explicit way to remove the whole isolated partition.
...
Local profiles and per-profile preferences live in a separate versioned, owner-readable state file. Profile creation and switching pass through sender-validated IPC. Each profile has its own Continue Watching document, lineup preferences, and bounded most-recently-launched service list, while streaming-service session partitions remain shared initially. Quick Launch reads only that service list, so opening or removing Continue Watching items does not manufacture app-recency data. On first launch after the profile migration, the legacy Continue Watching document is copied into the default profile without deleting the source.
...
Device preferences share the versioned local-state document but are not profile-scoped. The host owns display enumeration, window movement, fullscreen state, YouTube TV Mode, YouTube presentation scale, and ambient-display activation; the shell receives only display labels/counts and submits a narrow preference object. Safe-area, YouTube scale, reduced-motion, ambient clock style, and ambient delay choices are expressed as fixed enums rather than arbitrary CSS.
