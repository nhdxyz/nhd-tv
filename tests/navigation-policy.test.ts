...
const validDefinition: ServiceDefinition = {
  ...
  fullscreenOrigins: ["https://example.com"],
  id: "example",
  kind: "test",
...
describe("service navigation policy", () => {
  ...
  it("allows only explicitly declared service permissions and origins", () => {
    ...
    expect(isAllowedServicePermission("mediaKeySystem", "https://example.com/watch", validDefinition)).toBe(true);
    expect(isAllowedServicePermission("notifications", "https://example.com", validDefinition)).toBe(false);
  });
