...
TVmaze results are title discovery, not provider-availability claims. Each result offers explicit search actions for the enabled apps that support a safe search route. Netflix, YouTube, and Spotify accept the selected title as an allowlisted query; Disney+ opens its own search page. The service receives a title only after the user selects that action. The user makes the final catalog match inside the service.
...
- while Spotify is open, the query navigates that same service view to Spotify results with the text encoded into its search path;
...
As a convenience, selecting the qualified search field or launcher in Netflix, YouTube, or Spotify with the precision cursor opens this same bounded field and the phone's native keyboard. The capability is advertised while the cursor is still over the control so mobile browsers can focus the phone field during the user's tap. After the provider click, NHD-TV briefly rechecks for a lazily mounted declared input or textarea and binds that exact visible field. The phone is told only that the current safe target supports text entry; it never receives a DOM selector, field content, URL, or service-page text. In direct-entry mode, each bounded replacement value uses the provider's native input event path; submitting sends Enter to that same service view. Credential, authentication, payment, popup, custom-service, hidden, ambiguous, and arbitrary fields remain rejected.

Both navigation routing and direct provider typing occur through the trusted main process. Routing accepts only adapter-declared destinations; direct entry accepts only the adapter's declared selectors after independently rejecting sensitive form boundaries and non-text input types. Queries are not stored or logged. The TV UI discloses that two-character-or-longer Home queries are sent to TVmaze. iPhone Dictation and Gboard voice typing work in ordinary text fields, providing useful voice search without granting the LAN page microphone access.
