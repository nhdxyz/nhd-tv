...
The remote defaults to explicit arrows and offers a precision surface as an alternate input adapter. It sends only normalized coordinates in the closed `[0,1]` range, a bounded edge-scroll value, and a `move` or `tap` phase at a throttled rate. The host maps these to the visible shell or active service viewport, snaps to nearby visible controls, and applies the same high-contrast target marker used by directional navigation before forwarding a native click. Sign-in popups, credential/payment forms, hidden service views, and permission UI are excluded. No DOM selector, target text, URL, or page content is accepted from or returned to the phone.
