...
As a convenience, tapping the qualified search field in Netflix or YouTube with the precision cursor opens this same bounded field and the phone's native keyboard. The phone is told only that the current safe target supports text entry; it never receives a DOM selector, field content, URL, or service-page text. In this direct-entry mode, each bounded replacement value is applied only to the currently active or precision-marked declared `search`/`text` input using the provider's native input event path; submitting sends Enter to that same service view. Credential, authentication, payment, popup, custom-service, and arbitrary inputs remain rejected.
