...
To use a phone as a session-only remote, choose **Pair a phone** in the top bar or Settings, scan the short-lived QR code from a phone on the same trusted network, and approve the request on the TV. The full-height controller keeps icon-only Back and Home controls in its upper corners, defaults to directional arrow buttons, and can switch to an optional precision pad. Its luminous cursor and crosshair follow lightly smoothed normalized movement on both screens, dragging through an edge zone scrolls proportionally in the same direction as the finger, and nearby visible controls use target hysteresis before snapping into a high-contrast focus ring on the TV. Hover does not take DOM focus; a short tap selects the stable safe target. It also sends one bounded search phrase. While Netflix or YouTube is open, phone search stays in that service and opens its prefilled search results; Disney+ opens its search page. From Home, search uses the enabled-service chooser. Pointer routing cannot target editable, credential, payment, permission, popup, or hidden surfaces. Use the microphone on the phone's native keyboard for voice dictation. Supported phones provide light haptic confirmation after the TV accepts an action or locks onto a new target. Restarting NHD-TV revokes all paired phones.
