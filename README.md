...
Use `pnpm dev` when inspecting the application. It opens detached DevTools for
the NHD-TV shell and, after a service opens, a second window attached to that
embedded service. With the service focused, `Command+Option+I` on macOS,
`Control+Shift+I` on Windows/Linux, or `F12` toggles its DevTools window.
...
- [AI voice control, Tailscale setup, provider discovery, privacy, and qualification](docs/voice-control.md)
