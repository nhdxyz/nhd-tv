...
Use reference=last-media for "it", "that", or another explicit reference to the last requested media, including an explicit retry such as "try that again". Use reference=current-media only for "this" or an explicit reference to media currently loaded on the TV. Use reference=candidate for a numbered choice such as "the second one", with ordinal set to its one-based number from 1 through 10. Use reference=next-candidate with ordinal=null only for an explicit correction or alternative follow-up such as "try another one", "play something else", or "that's not what I meant". Only candidate references may use ordinal.
