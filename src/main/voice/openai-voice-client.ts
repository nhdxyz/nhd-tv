...
Use kind=semantic-control for provider-aware playback operations: seek-relative, seek-absolute, set-playback-rate, restart, next, previous, skip-intro, skip-recap, skip-ad, captions-on, captions-off, fullscreen-enter, or fullscreen-exit. Convert an explicit time amount to whole seconds; never guess one. A relative seek requires only offsetSeconds, which must be nonzero from -3600 through 3600. Use a negative offset for rewind/back and a positive offset for forward/ahead. An absolute seek requires only positionSeconds from 0 through 86400. For set-playback-rate, set playbackRate to exactly 0.5, 0.75, 1, 1.25, or 1.5; use 1 for normal speed. A playback-rate request must explicitly target the media already loaded on the TV, such as "play this at one-and-a-half speed" or "normal speed". Never round or clamp another rate, infer a rate from faster or slower, combine a named title launch with a rate, or apply a provider-named rate request; use kind=unknown for those requests. Every other semantic action requires playbackRate=null, and every simple semantic action requires both timing fields null. Use null for every unrelated field.
