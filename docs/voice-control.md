...
- Each media command is bound to the profile generation that created it. Slow discovery continually intersects its original candidates with the live lineup, so switching profiles fails closed and a newly enabled service cannot join an in-flight command. A server-owned authority gate serializes profile switches, built-in lineup removals, and custom-service removals: NHD-TV blocks new voice uploads and confirmation execution, cancels pending confirmations and the owning phone operation, aborts Google lookup, supersedes provider operations, closes a removed active provider, and only then commits the change. The gate remains closed across asynchronous custom-service cleanup and rechecks authority before reopening. Ordering, favorites, playback-mode changes, and additions do not broaden an in-flight command's frozen candidates.
...
- Netflix's profile gate selects one exact active local profile-name match. It may select a fallback only when there is exactly one safe normal profile; an absent or ambiguous match leaves the gate visible and immediately asks the user to choose on the TV rather than guessing or waiting through the retry deadline.
...
On a Netflix multi-profile gate, verify an exact local profile-name match, a single-profile fallback, and a missing-name multi-profile case. The missing-name case must leave every profile untouched, return the choose-profile instruction immediately, and succeed after the user chooses a profile and repeats the command.
